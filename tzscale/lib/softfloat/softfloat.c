...
INLINE int16 extractFloat64Exp( float64 a )
{
    ...
    return ((unsigned)(a.high)>>20 ) & 0x7FF;
...
float64 float64_round_to_int( float64 a )
{
    ...
    if ( 0x413 <= aExp ) {
        ...
        if ( roundingMode == float_round_nearest_even ) {
            if ( lastBitMask ) {
		unsigned zh, zl;
                add64( z.high, z.low, 0, lastBitMask>>1, zh, zl);
                z.high = zh;
		z.low  = zl;
		...
		unsigned zh, zl;
                add64( z.high, z.low, 0, roundBitsMask, zh, zl);
                z.high = zh;
		z.low  = zl;
            }
        }
        ...
        if ( aExp <= 0x3FE ) {
            ...
            switch ( float_rounding_mode ) {
             ...
             case float_round_down:
                if (aSign)
		    return packFloat64( 1, 0x3FF, 0, 0 );
		else
		    return packFloat64( 0, 0, 0, 0 );
             case float_round_up:
                if (aSign)
		    return packFloat64( 1, 0, 0, 0 );
		else
		    return  packFloat64( 0, 0x3FF, 0, 0 );
            }
...
float64 float64_rem( float64 a, float64 b )
{
    ...
    bits32 allZero, alternateASig0, alternateASig1, sigMean0, sigMean1;
    sbits32 ssigMean0;
    float64 z;
