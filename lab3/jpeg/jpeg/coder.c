...
INLINE void writebits_add(unsigned value, unsigned len)
{
...
/*INLINE*/ void jpeg_fdct_ifast (signed char* data_in, DCTELEM * data_out)
{
  ...
  for (int i=0; i<DCTSIZE; i++) chess_unroll_loop(*) {
#ifdef USE_CI_2
    tmp0 = data_in_ptr[0];
    tmp1 = data_in_ptr[1];
    tmp2 = data_in_ptr[2];
    ...
    jpeg_fdct(tmp0, tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7);
...
#ifdef USE_CI_2
    tmp0 = data_tmp_ptr[DCTSIZE*0];
    tmp1 = data_tmp_ptr[DCTSIZE*1];
    tmp2 = data_tmp_ptr[DCTSIZE*2];
    tmp3 = data_tmp_ptr[DCTSIZE*3];
    tmp4 = data_tmp_ptr[DCTSIZE*4];
    tmp5 = data_tmp_ptr[DCTSIZE*5];
    tmp6 = data_tmp_ptr[DCTSIZE*6];
    tmp7 = data_tmp_ptr[DCTSIZE*7];
    
    jpeg_fdct(tmp0, tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7);
    
    data_out_ptr[DCTSIZE*0] = tmp0;
    data_out_ptr[DCTSIZE*1] = tmp1;
    data_out_ptr[DCTSIZE*2] = tmp2;
    data_out_ptr[DCTSIZE*3] = tmp3;
    data_out_ptr[DCTSIZE*4] = tmp4;
    data_out_ptr[DCTSIZE*5] = tmp5;
    data_out_ptr[DCTSIZE*6] = tmp6;
    data_out_ptr[DCTSIZE*7] = tmp7;   
#else
  ...
  if (blk<4) {
	...
	for (int x2=0;x2<4;x2++) {
		...
		for (int x=0;x<2;x++) chess_unroll_loop(*) {
		    index = (height-1-(mby*16+by*8+y2*2+y))*width+(mbx*16+bx*8+x2*2+x);
		    BYTE R = pRGBbuf[index].R;
		    BYTE G = pRGBbuf[index].G;
		    BYTE B = pRGBbuf[index].B;
#ifdef USE_CI_1
		    DU[(y2*2+y)*8+(x2*2+x)] = DUCbCr(R,G,B,Cb,Cr);
#else
