...
-- File : tlx_longlong.h
--
-- Contents : Long long integer application layer for the TLX processor.
--
...
#ifndef _tlx_longlong_h
#define _tlx_longlong_h

// dual int representation of long long 

namespace tlx_primitive {
    struct dint property(keep_in_registers) {
        unsigned high;   // TLX is big endian hence MSBs first
        unsigned low;
        dint(unsigned h, unsigned l) : high(h), low(l) { }
    };
};
...
namespace tlx_primitive {
    promotion unsigned add(unsigned,unsigned,uint1&) property(duplicate_at_using_opn2) 
                               = w32 add(w32,w32,uint1&);
    promotion unsigned sub(unsigned,unsigned,uint1&) property(duplicate_at_using_opn2) 
                               = w32 sub(w32,w32,uint1&);

    promotion unsigned addx(unsigned,unsigned,uint1,uint1&)
                               = w32 addx(w32,w32,uint1,uint1&);
    promotion unsigned subx(unsigned,unsigned,uint1,uint1&)
...
inline   signed long long operator*(  signed long long a,  signed long long b) {
    return tlx_primitive::lmul_called(a,b);
}
inline unsigned long long operator*(unsigned long long a,unsigned long long b) {
    return tlx_primitive::lmul_called((long long)a,(long long)b);
}
