...
// declaration of primitive data types 
    ...
    class int10ns16   property( 10 bit signed, min=-512, max = -16, step =16);
    class int10ps16   property( 10 bit signed, min=16, max = 496, step =16);
    ...
    class int20p  property(20 bit signed); //right padded with zeros
    ...
    // CUSTOM types
    class uint1024  property(1024 bit unsigned);
    // end CUSTOM
...
// integer primitives 
    ...
    w32 lui(int20p) property(right_padding_12 promotion_conversion_alternate);
...
// CUSTOM primitives
    uint1024 REDC(uint1024, uint1024, uint1024);
    uint1024 add1024(uint1024, uint1024)    property(commutative);
    uint1024 sub1024(uint1024, uint1024);
    bool gt1024 (uint1024, uint1024);
    uint1024 div2 (uint1024);
